...
class Run_Bayesian:
    def runCsc(self, well_z_in, well_vp_in, td_z_in, td_t_in, par):
        ...
        zstart = par["zstart"]
        well_z, well_vp, td_z, td_t, well_z_top, well_vp_top, td_z_top, td_t_top = (
            redatum(well_z_in, well_vp_in, td_z_in, td_t_in, zstart)
        )
        ...
        ff = interp1d(
            np.insert(well_z_dec, 0, 0),
            np.insert(well_t_dec_post, 0, 0),
            kind="linear",
            bounds_error=False,
            fill_value=np.nan,
        )
        ...
        std_function = interp1d(
            np.insert(well_z_dec, 0, 0),
            np.insert(std_vel, 0, 0),
            kind="linear",
            bounds_error=False,
            fill_value=np.nan,
        )
        ...
        std_function = interp1d(
            well_z_dec, std_vel, kind="linear", bounds_error=False, fill_value=np.nan
        )
...
def makeG(well_z, td_z):
    ...
    # depth step
    well_dz = np.diff(
        np.insert(well_z, 0, 0)
    )  # assumes well_z starts depth > 0, inserts zero depth
...
def getDefaultPar():
    ...
    par["std_vp_perc"] = 0.5  # percentage (fraction)
    ...
    par["std_t_perc"] = 0.05  # percentage (fraction)
