...
def log_in():
    ...
    # KCSB = KustoConnectionStringBuilder.with_aad_managed_service_identity_authentication(KUSTO_CLUSTER, CLIENT-ID=AAD_TENANT_ID)
...
def load_els_data_fmb(df_sonic, selected_log_curve):
    try:
        ...
        # selected_source_welllog = 'LFP'
        # df_well_log['source'] = selected_source_welllog
        df_well_log["interval_velocity_sonic"] = [
            0.3048 / (float(sonic) * 0.000001) if sonic != 0 else 0
            for sonic in df_well_log[f"{selected_log_curve}"]
        ]
        return df_well_log
