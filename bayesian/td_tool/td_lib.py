...
def resample_tvdss_md(df_well, df_wellbore):
    ...
    df_well.loc[missing_md_indices, "md"] = md_interp(
        df_well.loc[missing_md_indices, "TVDMSL"]
    )
...
def extendLogsToZero(well_z, well_vp, water_depth, water_vel):
    ...
    kk = 0.5  # s^-1 # exponential rate of velocity increase
...
def applyTimeShift(well_z, well_t, td_z, td_t):
    ...
    if np.isnan(well_t0):
        ...
        t_shift = smod.getTimeFromTD(
            well_z[0], td_z, td_t
        )  # time at first log valuetd_drift
    ...
    if td_z[-1] < well_z[-1]:
        ...
        if any(ii):
            ...
            if dz > 0:
                ...
                zz_new = np.union1d(
                    zz_new, zz[-1]
                )  # ensure laste depth sample is included
                ff = interp1d(
                    zz, tt, kind="linear", bounds_error=False, fill_value=np.nan
                )
                tt = ff(zz_new)
                zz = zz_new
...
def getDrift(td_z, td_t, well_z, well_t):
    ...
    # drift (TWT --> OWT)
    well_drift_t = 0.5 * (well_t - well_td_t)
...
def ensureIncreasing(td):
    ...
    for w0 in wu:
        ...
        if len(kk[0]) > 0:
            k0 = kk[0][
                0
            ]  # index of first elements where time or depth is not strictly increasing
